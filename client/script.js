...
// CODE HERE
function sayHelloRevert() {
    sayHelloButton.style.backgroundColor = '#EFEFEF'
    sayHelloButton.style.color = 'black'
}
sayHelloButton.addEventListener('mouseout', sayHelloRevert)
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/dev').then((res) => {
        console.log(res.data)
...
// CODE HERE
const queryTest = () => {
    axios.get('http://localhost:3000/query-test?v=test&g=code').then((res) => {
        console.log(res.data)
        //Empty query returned "You sent an empty query"
        //multiple queries returned an object with both queries in them
    })
}
...
function createFood(event) {
    ...
    let foodInput = document.getElementById('input-text').value
    let body = {
        newFood: foodInput,
    }
    axios.post('http://localhost:3000/food', body).then((res) => {
        var foodVar = document.createElement('p')
        foodVar.textContent = res.data[res.data.length - 1]
        document.body.appendChild(foodVar)

    })
}
document.getElementById('submit-button').addEventListener('click', createFood)